...
| CI/CD | `ci.yml` runs tests only (no lint/type-check, see Backend above) on every push/PR; `deploy.yml` uses Workload Identity Federation (no long-lived service account key) to build and deploy to Cloud Run, but is **manually triggered only** (`workflow_dispatch`) — it can't run automatically on push because it depends on secrets that only exist once someone has actually run `terraform apply` against a real GCP account | Trunk-based development with feature flags and canary/progressive delivery instead of an all-at-once deploy; DORA metrics to track the health of the delivery process itself |
